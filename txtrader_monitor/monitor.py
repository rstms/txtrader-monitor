...
class Monitor(object):
    ...
    def __init__(self, host=None, port=None, user=None, password=None, callbacks=None):
        """Initialize Monitor:
    ...
    def listen(self, _reactor):
        ...
        self.connection = reactor.connectTCP(self.host.encode(), self.port, f)
    ...
    def stringReceived(self, data):
        ...
        if data.startswith('.'):
            ...
            if data.startswith('.connected'):
                self.sendString(
                    ('auth %s %s %s' % (self.factory.rx.user, self.factory.rx.password, self.factory.rx.flags)).encode()
                )
